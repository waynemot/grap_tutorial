module CalendarHelper
  ...
  def get_all_calendar_groups(token)
    get_calendar_groups_url = '/v1.0/me/calendarGroups'
    #query = {'expand' => :children}
    response = make_api_call "GET", get_calendar_groups_url, token #, {}, query
    raise response.parsed_response.to_s || "Request returned #{response.code}" unless response.code == 200

    response.parsed_response['value']
  end

  def get_calendars_for(token, calendar_id)
  ...
  def get_photo(token)
    get_photo_url = "/v1.0/me/photos/48x48/$value"
    headers = {'Content-Type'=> 'image/jpg'}

    response = make_api_call 'GET', get_photo_url, token
    raise response.parsed_response.to_s || "Request returned #{response.code}" unless response.code == 200
